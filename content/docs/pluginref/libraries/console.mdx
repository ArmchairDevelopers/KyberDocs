...
```lua linkTypes
Console.GetSettings(category: string): DataContainer
```
...
```lua linkTypes
Console.Execute(command: string): void
```
...
```lua linkTypes
Console.Register(group: string, name: string, description: string, inst: table, callback: function): void
```
